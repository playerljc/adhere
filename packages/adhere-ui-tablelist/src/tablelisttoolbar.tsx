...
export const ToolbarSelectAll = ({ selectAll, rowSelection, rowKey, dataSource, setSelectAll }) => {
  ...
  function onChange(e) {
    if (e.target.checked) {
      rowSelection.onChange(dataSource.map(v => v[rowKey]), dataSource);
    } else {
    ...
    <Tooltip title={intl.v('全选')} placement="top" {...selectAll}>
      {
        <Checkbox
          indeterminate={
            selectAll.total && rowSelection.selectAll ? rowSelection.selectAll?.exceptKeys?.length :
            rowSelection.selectedRowKeys.length && rowSelection.selectedRowKeys.length !== dataSource?.length
          }
          checked={selectAll.total && rowSelection.selectAll ? !rowSelection.selectAll?.exceptKeys?.length : rowSelection.selectedRowKeys.length === dataSource?.length}
          onChange={onChange}
        >
...
export const ToolbarSetting = ({ setting, tableColumns, onSettingChange, onSettingSortEnd, selectedColumnKeys }) => {
      ...
      >
        {
          columns.map((item: any, index) => (
            <SortableItem value={item.key} index={index} key={item.key}>{item.title}</SortableItem>
          ))
        }
      </SortableWrapper>
    )
  ...
  return (
    <Popover
      title={<SettingTitle columns={tableColumns} onChange={onSettingChange} selectedColumnKeys={selectedColumnKeys} />}
      content={<SettingContent columns={tableColumns} onChange={onSettingChange} onSortEnd={onSettingSortEnd} selectedColumnKeys={selectedColumnKeys} />}
      trigger="click"
