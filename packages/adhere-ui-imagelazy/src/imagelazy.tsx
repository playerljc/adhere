...
class ImageLazy extends React.Component<IImageLazyProps, any> {
  static defaultProps: {
    imgArgs: { originSrc: string; targetSrc: string };
    className: string;
  };

  static propTypes: { imgArgs: any; className: any };

  render() {
    return (
      <img
        src={this.props.imgArgs.originSrc || placeholder}
        data-src={this.props.imgArgs.targetSrc}
        className={classNames('lazyload', ...(this.props.className || '').split(/\s+/))}
      />
    );
  }
}
