...
export class Events {
   ...
   */
  private events: Record<EventType, EventHandlerEntry> = {};
  ...
  remove(type: EventType, handler: EventHandler): boolean {
    ...
    const handlers = this.events[type].handlers;
  ...
  hasType(type: EventType): boolean {
    return type in this.events && this.events[type].handlers.length > 0;
  }
   ...
   * ```
   */
  on(type: EventType, handler: EventHandler, maxStackSize: number = 200): () => boolean {
    if (typeof handler !== 'function') {
      throw new TypeError('Handler must be a function');
    }

    if (maxStackSize < 1) {
      throw new RangeError('maxStackSize must be at least 1');
    }
    ...
    if (handlers.length >= maxStackSize) {
      handlers.shift();
    }

    // If somehow we have more handlers than maxStackSize, trim to size
    if (handlers.length > maxStackSize) {
      const { length } = handlers;
      this.events[type].handlers = handlers.slice(length - maxStackSize);
    }
  ...
  once(type: EventType, handler: EventHandler): () => boolean {
    ...
    this.events[type].handlers = [wrappedHandler];

    // Return unsubscribe function
    return () => this.remove(type, wrappedHandler);
  }
  ...
  all(types: EventType[], handler: EventHandler): () => void {
    ...
    types.forEach((type) => {
      const key = typeof type === 'string' ? Symbol.for(type) : type;
      ...
      const fn: EventHandler = (...args) => {
        ...
        if (changeLogs.every((t) => t.status)) {
          if (handler) {
            // Reset all statuses for next round
    ...
    if (this.hasType(type)) {
      ...
      for (const handler of handlers) {
        try {
          ...
          console.error(`Error in event handler for type '${String(type)}':`, error);
          // Continue executing other handlers even if one fails
        }
      }

             // If type is string, also trigger the symbol version
       if (typeof type === 'string') {
         const symbolType = Symbol.for(type);
         // Avoid infinite recursion by checking if we're already handling a symbol
         this.trigger(symbolType, ...params);
       }
    }
  ...
  getHandlerCount(type: EventType): number {
    if (this.hasRegisteredType(type)) {
      return this.events[type].handlers.length;
    }
    return 0;
  }
