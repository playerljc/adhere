...
export default {
     ...
     * @return
     */
    fillCityBoundary(cityName, style, map) {
        const cityOverlays = [];
        const cityPoints = [];
    ...
    clientToCtxPointToEl({ event, rect, offsetEl, }) {
        let { left, top } = typeof window !== 'undefined'
            ? window.getComputedStyle(offsetEl, null)
            : {
                left: '0',
                top: '0',
            };
        const offsetLeft = parseInt(left.replace('px', ''));
        const offsetTop = parseInt(top.replace('px', ''));
