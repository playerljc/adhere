...
class DistanceDrawAction extends PolygonDrawAction {
    ...
    onCursorMouseMove(e) {
        ...
        ctx.fillStyle = '#000';
        ctx.fillText(`${Intl.v('总长')}：`, textX - distanceTextHalfWidth, textY, ctx.measureText(`${Intl.v('总长')}：`).width);
        // 数值
        ctx.beginPath();
        ctx.font = textFont;
        ...
        ctx.textBaseline = textBaseline;
        ctx.fillStyle = '#000';
        ctx.fillText(Intl.v('单击确定地点，双击结束'), textX, toolTipPixel.y + (radiusRectHeight / 4) * 3 + 3, ctx.measureText(Intl.v('单击确定地点，双击结束')).width);
    }
    /**
    ...
    fill() {
        ...
        originData.forEach((point, index) => {
            DistanceDrawAction.drawStartPoint({
                context,
                ctx,
                style: DistancePointStyle,
