...
class RectangleDrawAction extends DrawAction {
    ...
    static drawHistoryPath(context, ctx, data) {
        ctx.save();
        ctx.beginPath();
        const realData = RectangleDrawAction.transformOriginToReal(context, data);
        ctx.rect(realData?.leftTopPoint?.x || 0, realData?.leftTopPoint?.y || 0, realData?.width, realData?.height);
        // 描边
        ctx.stroke();
        // 填充
        ctx.fill();
