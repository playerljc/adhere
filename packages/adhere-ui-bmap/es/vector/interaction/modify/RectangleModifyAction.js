...
class RectangleModifyAction extends ModifyAction {
    ...
    drawAnchors() {
        ...
        if (!ctx)
            return;
        const { leftTopPoint, width, height } = RectangleDrawAction.transformOriginToReal(this.context, this?.data?.data?.data);
    ...
    getBox() {
        ...
        if (!data)
            return null;
        const { leftTopPoint, width, height } = RectangleDrawAction.transformOriginToReal(context, data.data);
        return {
    ...
    drawMoveGeometry() {
        if (!this.context || !this.data)
            return;
        RectangleDrawAction.draw(this.context, this.context.getAssistCtx(), this.data);
    }
    // @ts-ignore
    drawMoveGeometry(startPixel, targetPixel) {
        ...
        if (srcData.data && srcData.data.leftTopPoint) {
            ...
            RectangleDrawAction.draw(this.context, this.context.getAssistCtx(), srcData);
        }
    }
    destroy() {
        this.startIndex = -1;
