...
class CircleModifyAction extends ModifyAction {
    ...
    drawAnchors() {
        ...
        if (!ctx)
            return;
        const { center, radius } = CircleDrawAction.transformOriginToReal(this.context, this?.data?.data?.data);
        // 顺时针，上，右，下，左
    ...
    drawModify(targetPixel) {
        ...
        data.data.radius = context.distanceToActual(MathUtil.getDistanceByBetweenPoint({ p1: centerPixel, p2: targetPixel }));
        this.data.data = {
            ...data,
        };
        context.clearDraw();
    ...
    isCanMove(targetPixel) {
        ...
        const inCircle = MathUtil.isPointInCircle(targetPixel, CircleDrawAction.transformOriginToReal(this.context, {
            center,
            radius,
        }));
    ...
    drawMoveGeometry() {
        if (!this.context || !this.data)
            return;
        CircleDrawAction.draw(this.context, this.context.getAssistCtx(), this.data);
    }
    /**
     * drawMoveGeometry
     * @description 绘制移动时的几何图形
    ...
    drawMoveGeometry(startPixel, targetPixel) {
        ...
        if (srcData.data && srcData.data.center) {
            ...
            CircleDrawAction.draw(this.context, this.context.getAssistCtx(), srcData);
        }
    }
    setCursor() { }
}
