...
let maxValue = 0;
if (data.length) {
    maxValue = 0.9 / (Math.max(...(data.map(item => item.accidentNum || 0).concat(data.map(item => item.dieNum || 0)))));
}
...
// 柱状体的主干
const lineTrunk1 = () => {
    return data.filter((item) => item.accidentNum).map((item) => {
        const val1 = Number(item.centroid[0]);
        const val2 = Number(item.centroid[1]);
        return {
            coords: [[val1 - 0.06, val2], [val1 - 0.06, val2 + item.accidentNum * maxValue]]
        }
    })
}
...
// 柱状体的主干
const lineTrunk2 = () => {
    return data.filter((item) => item.dieNum).map((item) => {
        const val1 = Number(item.centroid[0]);
        const val2 = Number(item.centroid[1]);
        return {
            coords: [[val1 + 0.06, val2], [val1 + 0.06, val2 + item.dieNum * maxValue]]
        }
    })
}
        ...
        // 柱状体的顶部
        {
            ...
            itemStyle: {
                color: {
                    ...
                    r: 0.5,
                    colorStops: [{
                        offset: 0, color: '#4B26F6' // 0% 处的颜色
                    }, {
                        offset: 1, color: '#4D25FD' // 100% 处的颜色
                    }],
                    global: false // 缺省为 false
                },
                opacity: 1
