# @baifendian/adhere-util-iframeio

一个强大的 iframe 通信工具库，提供客户端和服务端之间的双向通信功能。

## 📖 简介

`adhere-util-iframeio` 是一个专门用于处理 iframe 通信的工具库，它提供了类似 HTTP 请求/响应的通信模式，支持路由、中间件、文件上传等功能。适用于需要在不同域名或页面间进行安全通信的场景。
...
// 创建客户端
const fetch = new IframeIO.Fetch(window, sourceOrigin);

// 创建服务端
const server = new IframeIO.Server(whitelist, window, sourceOrigin);
```
...
// 创建客户端实例
const fetch = new IframeIO.Fetch(window, window.location.origin);
...
// 创建路由
const router = new IframeIO.Router();
...
// 创建服务端
const server = new IframeIO.Server(
...
import { useFetch, useServer } from '@baifendian/adhere-util-iframeio';
...
- [在线演示](https://playerljc.github.io/adhere/index.html#/adhere/adhere/util/iframeio)
