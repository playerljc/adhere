...
const menuItemPrefix = `${SELECT_PREFIX}-design-toolbar-menu-preview`;
  ...
  return (
    <>
      <span
        ...
        title={canShow ? config.label : Intl.get('preview_disabled_hint')}
        aria-disabled={!canShow}
        onClick={canShow ? handleClick : undefined}
      >
        {config.icon} {config.label}
      </span>
      {open ? <PreviewModal open={open} onClose={handleClose} /> : null}
    </>
  );
};
