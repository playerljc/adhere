...
const selectorPrefix = `${SELECT_PREFIX}-upload-data-source-manager-form-item`;

/**
 * UploadDataSourceManagerFormItem
 * @description 上传组件的数据源选择：左侧回显摘要，右侧选择/管理
 */
const UploadDataSourceManagerFormItem: FC<UploadDataSourceManagerFormItemProps> = ({
  value,
  onChange,
}) => {
    ...
    <>
      <div className={selectorPrefix}>
        <div
          ...
          title={summary || Intl.get('upload_data_source_placeholder')}
        >
          {summary || Intl.get('upload_data_source_placeholder')}
        </div>

        <div className={`${selectorPrefix}-actions`}>
          <Select
            className={`${selectorPrefix}-select`}
            placeholder={Intl.get('upload_data_source_select')}
